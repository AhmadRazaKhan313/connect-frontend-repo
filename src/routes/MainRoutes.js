...
// Basic: sirf login check
const checkLogin = (element) => {
    if (jwt.getIsLogin() !== true) return <Navigate to="/login" replace />;
    return element;
};
...
const checkPlatformSA = (element) => {
    if (jwt.getIsLogin() !== true) return <Navigate to="/login" replace />;
    if (!isPlatformSA()) return <Navigate to="/dashboard/forbidden" replace />;
    return element;
};
...
const MainRoutes = {
    ...
    children: [
        ...
        // ── Organizations — PLATFORM SUPER ADMIN ONLY ────────────────────────
        { path: '/dashboard/add-organization',          element: checkPlatformSA(<AddOrganization />) },
        { path: '/dashboard/all-organizations',         element: checkPlatformSA(<AllOrganizations />) },
        { path: '/dashboard/edit-organization/:id',     element: checkPlatformSA(<EditOrganization />) },

        // ── Error ─────────────────────────────────────────────────────────────
        { path: 'forbidden',                            element: checkLogin(<Forbidden />) },
